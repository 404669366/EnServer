...
class Tld
{
    ...
    {
        $time = substr($buffer, 0, 1);
        $time .= substr($buffer, 1, 1) . '-';
        $time .= substr($buffer, 2, 1) . '-';
        $time .= substr($buffer, 3, 1) . ' ';
        $time .= substr($buffer, 4, 1) . ':';
        $time .= substr($buffer, 5, 1) . ':';
        $time .= substr($buffer, 6, 1);
        return $time;
    }
